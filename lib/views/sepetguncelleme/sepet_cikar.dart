...
final barcodeProviderr = StateProvider<String?>((ref) => null);
...
class SepetCikar extends ConsumerStatefulWidget {
  const SepetCikar({super.key});

  @override
  ConsumerState<SepetCikar> createState() => _SepetCikarState();
}

class _SepetCikarState extends ConsumerState<SepetCikar> {
  @override
  void initState() {
    super.initState();
    _initializeScanner();
    ...
    if (await PMScanner.isDevicePointMobile()) {
      ...
      PMScanner.onDecode = (Symbology symbology, String barcode) {
        if (barcode.isNotEmpty) {
          ...
          ref.read(barcodeProviderr.notifier).state = barcode;
        }
      };
    } else {
      showDialog(
  ...
  Widget build(BuildContext context) {
    final barcodeNo = ref.watch(barcodeProviderr); // Barkod değerini izliyoruz
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    if (barcodeNo != null && barcodeNo.isNotEmpty) {
                      ...
                      // API isteğini yap
                      final success = await orderServices.cikarSepet(userId, barcodeNo);

                      if (success) {
                        // Snackbar ile başarı mesajı göster
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text("Sepet başarıyla çıkarıldı."),
                            backgroundColor: Colors.green,
                          ),
                        );

                        // Barkod numarasını sıfırla
                        ref.read(barcodeProviderr.notifier).state = null;
                      } else {
                        // Snackbar ile hata mesajı göster
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text("Sepet çıkarılamadı."),
